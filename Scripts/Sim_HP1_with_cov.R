...
res <- datos %>% 
  ...
  drop_na() %>% 
  group_by(n) %>% 
  summarise(mean_b0_mu=mean(b0_mu_hat, trim=0.15),
            mean_b1_mu=mean(b1_mu_hat, trim=0.15),
            mean_b0_si=mean(g0_si_hat, trim=0.15),
            mean_b1_si=mean(g1_si_hat, trim=0.15),
